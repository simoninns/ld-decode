...
The measurement helpers run over fields from two sample domains: one loaded
from a .cvbs, in the normative 10-bit domain video_common.load_cvbs returns,
and one built straight from the decoder's in-memory output by
video_frames.frames_from_decoded, which is 16-bit.  The two differ by roughly
a factor of 64, so every measurement reported has to be a ratio against the
field's own out_scale; anything left in raw sample units would read 64x wrong
on one of the two.
...
def synthetic_field(system, blanking, white, burst_ire, seed=12345):
    ...
    field and 16-bit values for a decoder-domain one.  The waveform is
    identical in IRE either way, so a domain-independent measurement returns
...
def domain_pair(system, burst_ire, seed=12345):
    """The same waveform as a 10-bit .cvbs field and a 16-bit decoder-domain
    field."""
    ten_bit = CaptureParams(system)
    cvbs = synthetic_field(
        system, ten_bit.blanking_16b_ire, ten_bit.white_16b_ire, burst_ire, seed)
    decoder_domain = synthetic_field(
        system, ten_bit.blanking_16b_ire * 64, ten_bit.white_16b_ire * 64,
        burst_ire, seed)
    return cvbs, decoder_domain
...
def test_burst_rms_is_the_same_in_both_sample_domains(system, burst_ire):
    cvbs, decoder_domain = domain_pair(system, burst_ire)
    np.testing.assert_allclose(
        measure_burst(cvbs), measure_burst(decoder_domain), rtol=1e-9)
...
def test_burst_3d_residue_is_the_same_in_both_sample_domains(system):
    ...
    cvbs_a, decoder_a = domain_pair(system, 20.0, seed=1)
    cvbs_b, decoder_b = domain_pair(system, 20.0, seed=2)
    np.testing.assert_allclose(
        measure_burst_3d(cvbs_a, cvbs_b),
        measure_burst_3d(decoder_a, decoder_b),
        rtol=1e-9,
    )
